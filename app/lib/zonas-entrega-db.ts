  ...
  return snapshot.docs
    .map((item) => {
      ...
      return {
        ...
        montoMinimoGratis: Number(data.montoMinimoGratis ?? 25),
        zonas: Array.isArray(data.zonas)
          ? data.zonas.map((zona: ZonaEntrega) => ({
              ...
              ...(zona.cobroFijo === undefined ? {} : { cobroFijo: Number(zona.cobroFijo) || 0 }),
            }))
          : [],
      } as CiudadEntrega;
    })
...
export async function guardarCiudadEntrega(ciudad: Omit<CiudadEntrega, "id">, id?: string) {
  ...
  await setDoc(doc(db, CITIES_COLLECTION, cityId), {
    ...
    montoMinimoGratis: Math.max(0, Number(ciudad.montoMinimoGratis) || 25),
    zonas: ciudad.zonas.map((zona) => {
      ...
      if (zona.cobroFijo !== undefined) savedZone.cobroFijo = Math.max(0, Number(zona.cobroFijo) || 0);
      return savedZone;
    }),
  });
  return cityId;
...
export async function guardarZonaEntrega(cityId: string, zona: ZonaEntrega) {
  ...
  if (zona.cobroFijo !== undefined) nextZona.cobroFijo = Math.max(0, Number(zona.cobroFijo) || 0);
